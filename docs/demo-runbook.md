...
under **Authorized JavaScript origins** on the web OAuth client; check that before recording, not
on the day. The button's language follows the signed-in Google account, so a Google set to German
