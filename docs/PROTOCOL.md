...
> ⚠️ **UNRESOLVED CONFLICT — a human needs to pick one. Do not build against this section until
> then.** This section was rewritten to the parent-authored `BreakMessage` / `BreakPeriod` design
> while the gateway was being built to the previous `MovementBreak` / `BreakTask` design in the same
> hours. **The server on `agents/` today implements the older one**: `MovementBreak {id, kid_id,
> started_at, ends_at, seconds_left, task: {title, steps[], seconds, spoken}, source_titles[],
> acked}`, generated by a Coach agent from the child's own video titles and passed through a
> safety gate in code (`heygilli_agents/breaks.validate`) that drops anything mentioning climbing,
> furniture, running, stairs, outdoors, water, kitchens, sharp objects, fast spinning or needing an
> adult, falling back to a built-in task. It has no `break_is_firm`, no `break-messages` endpoints
> and no `BreakPeriod`. The two designs disagree on one real question — may a model's words reach a
> child at all — and that is a product call, not a merge. The accounting, the 409s, `PATCH /limits`,
> `GET /state`, `ack`, `override` and the socket's `break` message are identical either way; only
> the payload inside the break differs.
