...
def test_a_later_run_fills_in_lengths_screened_before_they_were_known(
    store: LocalStore, monkeypatch
) -> None:
    """Screening happens once and stores what was known then. Everything
    decided before a household could look a length up sits at 0 for ever, and 0
    is exactly what makes a parent's "longest video" limit unenforceable."""
    kid = Kid(household_id="hh", nickname="Abu", age=5, languages=["en"])
    store.put_kid(kid)
    store.put_channel("hh", kid.id, Channel(id="UCx", title="SciShow Kids"))
    monkeypatch.setattr(curator, "fetch_uploads", lambda cid, limit: UPLOADS)
    monkeypatch.setattr(curator, "fetch_video_meta", lambda vid: {"duration_s": 0, "thumb_url": "t"})
    monkeypatch.setattr(
        curator, "fetch_transcript",
        lambda vid: {"video_id": vid, "source": "captions:en:auto", "segments": SEGMENTS},
    )
    ...
    # First run: no Google grant, so no length can be found.
    monkeypatch.setattr(curator, "_durations", lambda hid, ids, store: {})
    model = FakeModel()
    curator.run_curator(kid, store, curator=make_agent("curator", "s", model=model),
                        planner=make_agent("planner", "s", model=model))
    assert store.get_video("goodvideo01").duration_s == 0

    # Later, the household links Google and a run can look them up. The videos
    # already decided must be filled in too, not only the unseen ones.
    monkeypatch.setattr(curator, "_durations", lambda hid, ids, store: {i: 900 for i in ids})
    curator.run_curator(kid, store, curator=make_agent("curator", "s", model=model),
                        planner=make_agent("planner", "s", model=model))
    assert store.get_video("goodvideo01").duration_s == 900

    # A length already known is never overwritten by a later lookup.
    monkeypatch.setattr(curator, "_durations", lambda hid, ids, store: {i: 1 for i in ids})
    curator.run_curator(kid, store, curator=make_agent("curator", "s", model=model),
                        planner=make_agent("planner", "s", model=model))
    assert store.get_video("goodvideo01").duration_s == 900
